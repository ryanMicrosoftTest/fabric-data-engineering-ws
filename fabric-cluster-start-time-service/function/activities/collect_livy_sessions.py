...
@app.function_name(name="collect_livy_sessions")
@app.activity_trigger(input_name="payload")
async def collect_livy_sessions(payload: dict) -> dict:
    ...
    try:
        ...
        log.info("collect_livy_sessions ok wid=%s rows=%s", wid, rows_written)
        return {"success": True, "rows_written": rows_written, "duration_ms": duration_ms}
    except FabricApiError as e:
        duration_ms = int((time.monotonic() - started) * 1000)
        log.error("collect_livy_sessions fabric error wid=%s: %s", wid, e)
        return {"success": False, "rows_written": 0, "error": str(e), "duration_ms": duration_ms}
    except Exception as e:  # noqa: BLE001
        duration_ms = int((time.monotonic() - started) * 1000)
        log.exception("collect_livy_sessions failed wid=%s", wid)
        return {"success": False, "rows_written": 0, "error": str(e), "duration_ms": duration_ms}
