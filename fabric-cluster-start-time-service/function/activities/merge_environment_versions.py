...
@app.function_name(name="merge_environment_versions")
@app.activity_trigger(input_name="payload")
def merge_environment_versions(payload: dict) -> dict:
    ...
    try:
        with WarehouseWriter(settings, credential) as wh:
            rowcount = wh.execute_proc("dbo.usp_merge_environment_versions", {"collector_run_id": cri})
        duration_ms = int((datetime.now(timezone.utc) - started).total_seconds() * 1000)
        logger.info(
            "merge_environment_versions complete",
        ...
        duration_ms = int((datetime.now(timezone.utc) - started).total_seconds() * 1000)
        logger.exception("merge_environment_versions failed")
        return {
            "success": False,
            "rows_written": 0,
