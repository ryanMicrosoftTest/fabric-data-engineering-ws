...
def test_happy_path(patched_module):
    ...
    writer.execute_proc.return_value = 7

    result = mod.merge_environment_versions({"collector_run_id": "run-1"})
...
def test_failure_returns_error(patched_module):
    ...
    result = mod.merge_environment_versions({"collector_run_id": "run-2"})
