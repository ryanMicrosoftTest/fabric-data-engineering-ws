# Fabric Workspace Private Link — Complete Pipeline

Automated Azure DevOps pipeline that deploys a Fabric workspace-level Private Link and sets the workspace inbound policy to deny public access.
...
4. **Workspace inbound policy** set to deny public access via the Fabric REST API
...
| **Deploy** | Runs deployment script: creates PLS + PE + DNS records + denies public access |
...
> **Note:** The workspace inbound policy (deny public access) can take up to 30 minutes to take effect.
...
| `deploy-workspace-private-link.sh` | Deployment script with validation and inbound policy |
