#!/bin/bash
# deploy-workspace-private-link.sh (Complete variant)
# Deploys a Fabric workspace-level Private Link Service + Private Endpoint,
# validates subnet capacity, and sets workspace inbound policy to deny public access.
#
# Usage:
...
set_workspace_inbound_policy() {
    log "Setting workspace inbound policy to deny public access..."

    TOKEN=$(az account get-access-token \
        --resource "https://api.fabric.microsoft.com" \
        --query accessToken \
        -o tsv 2>/dev/null) || fail "Failed to acquire Fabric API access token."
    ...
    if [[ "$HTTP_STATUS" -ge 200 && "$HTTP_STATUS" -lt 300 ]]; then
        log "Workspace inbound policy set to deny public access (HTTP $HTTP_STATUS)."
        log "NOTE: This setting can take up to 30 minutes to take effect."
    else
        log "WARNING: Failed to set workspace inbound policy (HTTP $HTTP_STATUS)."
        log "You may need to set this manually in Fabric Portal > Workspace Settings > Inbound Networking."
    fi
}
...
main() {
    log "=== Fabric Workspace Private Link Deployment (Complete) ==="
