...
# # Retrieve the execution-steps table

# CELL ********************

steps_table_name = f'{table_name}_steps'

steps_df = spark.table(steps_table_name)
...
client = FabricOpenAIResponses(
    artifact_name = 'clinical_data_agent',
    workspace_name=workspace_name,
...
question = (
    'What doctors work in the Cardiology department'
)
...
if response.status != 'completed':
    raise RuntimeError(f'Agent did not complete: {response.status}')

print('ANSWER')
print(response.output_text)

# inspect returned output items, including any exposed tool calls
response_payload = response.model_dump(mode='json')
output_items = response_payload.get('output', [])

steps_df = pd.json_normalize(output_items)
display(steps_df[['name', 'arguments', 'output','status']])

"""
print('\nRESPONSE OUTPUT ITEMS')
display(pd.json_normalize(output_items))

# retrieve diagnostics into a python variable
diagnostics = client.diagnostics.get(
    conversation.id,
    response_id = response.id 
)

print('n\CONVERSATION DIAGNOSTICS')
...
question_list = [
    ...
    "How many patients have notes?",
    "How many patient notes does patient_4 have",
    "Give me the full memo for the most recent memo for patient_5"
]


for question in question_list:
    
    # conversation is inside the loop to represent difference conversations initialized.  If outside, all questions are tied to the same conversation
    conversation = client.conversations.create()
