# Fabric benchmark notebook: 00_generate_tpcds
# Kernel: jupyter
...
# %% [markdown]
# # 00 - Generate TPC-DS source data
# Generates the TPC-DS subset with DuckDB's built-in `tpcds` extension and writes ZSTD Parquet into `lh_bench_source/Files/tpcds/<size>/<table>`.
# 
# **Default lakehouse must be `lh_bench_source`.** Scale factors are estimates (SF100 ~= 12.7 GB compressed per Miles Cole); calibrate with notebook 01. SF400 (~50 GB) is heavy on a single node — generate it only when ready.
