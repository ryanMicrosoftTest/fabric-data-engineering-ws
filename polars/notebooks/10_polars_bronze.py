# Fabric benchmark notebook: 10_polars_bronze
# Kernel: jupyter
...
# %% [markdown]
# # 10 - Polars Bronze (ingest)
# Reads TPC-DS Parquet from the **default lakehouse (`lh_bench_source`)**, adds ingest metadata, and writes Delta to `lh_bench_bronze` under schema `polars_<size>` with the pinned runtime's native streaming `sink_delta`; the full frame is never materialized. Narrow, scan-heavy, low-shuffle work — Polars' sweet spot.
# 
