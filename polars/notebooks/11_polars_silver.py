...
# %%
%pip install --quiet --disable-pip-version-check polars==1.42.1 deltalake==1.6.2
...
# Reads Bronze Delta from the **default lakehouse (`lh_bench_bronze`)**, dedupes the narrow fact on its natural key **before** joining it to its dimensions, and writes `silver.sales_conformed` to Delta. dedupe-then-join keeps the dedupe hash table on the small projected fact and every join build side dim-sized; the plan runs through the pinned runtime's native streaming `sink_delta`. Shuffle-heavy — where Spark starts to win.
# 
